...
we use two indexes: vectorIndex and bm25Index to search and find the most relevant section of the chunked document to the use's query:
for both, we perform the operations as below. (pls note that for b25, we don't need to generate embedding for chunks, we'll skip steps 2 & 4)
# 1. Chunk the text by section
# 2. Generate embeddings for each chunk
# 3. Create a vector store and add each embedding to it
        store = VectorIndex() or bm25
        store.add_documents([{"content": chunk} for chunk in chunks])
# 4.  Generate an embedding for user's question
...
from chunking_rag import chunk_by_section
import math
...
def generate_embedding(texts, model="text-embedding-3-small"):
    ...
    inputs = texts if is_list else [texts]
...
# Retriever implementation that mixes the result of the search from both indexes

class SearchIndex(Protocol):
    def add_document(self, document: Dict[str, Any]) -> None: ...

    # Added the 'add_documents' method to avoid rate limiting errors from embedding API
...
results=retriever.search("what happened with INC-20230Q4-011?",3)
